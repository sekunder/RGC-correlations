    ...
    if size(subdf, 1) >= 10
        ...
        for N = 10:min(20, N_neurons)
            ...
            for (nt,X) in zip([subdf[1,:neuron_type],"simulated"], [X_real, X_sim])
                ...
                if any(expectation_matrix(P_N) .<= 0)
                    println(lf, "$(ts())   ! WARNING some pair(s) of bits never fire together.")
                    println(lf, "$(st())     Appending all-ones vector to data (N_samples = $(size(X,2)))")
                    X = hcat(X, trues(size(X,1)))
                    P_N = data_model(X, neurons, verbose=2)
                end
                # TODO let's print some statistics/information about the data
                println(lf, "$(ts())     P_N(0) = $(pdf(P_N, falses(n_bits(P_N))))")
