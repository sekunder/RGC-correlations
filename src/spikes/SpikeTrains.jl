...
type SpikeTrains
    ...
    function SpikeTrains(TT, I=1:length(TT);
        rec_start=0.0, rec_finish=maximum(map(maximum(TT))),
        ...
        trains = [map(Float64, T[i][rec_start .< T[i] .< rec_finish]) for i in I]
...
function show(io::IO, ST::SpikeTrains)
    println("Spike times for $(n_cells(ST)) neurons.")
    println("Recording start/finish: $(ST.rec_start) / $(ST.rec_finish) ")
    println("Stimulus start/finish:  $(ST.stim_start) / $(ST.stim_finish)")
    show_metadata(io,ST)
end
...
function histogram(ST::SpikeTrains, binsize::Float64; t0::Float64=ST.stim_start, tmax::Float64=ST.stim_finish)
    ...
    spike_hist = zeros(N_frames, N_cells)
